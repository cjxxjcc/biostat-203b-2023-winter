...
```{r}
#| eval: true
system.time(time1 <- read.csv(paste0(mimic_path,"/core/admissions.csv.gz")))
```
```{r}
#| eval: true
system.time(time2 <- read_csv(paste0(mimic_path, "/core/admissions.csv.gz"),show_col_types = FALSE))
```
...
```{r}
icustays_tble <- read_csv(file.path(mimic_path, "/icu/icustays.csv.gz"),show_col_types = FALSE)
...
unique_subjects
```
**Solution**: There are total 51350 uniqye 'subject_id'. and for one subject_id, it can have multiple ICU stays.
...
```{r}
admissions_tble <- read_csv(file.path(mimic_path, "/core/admissions.csv.gz"),show_col_types = FALSE)
