...
```{r}
#| eval: true
system.time(time2 <- read_csv(str_c(mimic_path, "/core/admissions.csv.gz"),show_col_types = FALSE))
```
...
mimic_icu_cohort <- icustays_tble %>%
    ...
    print(width = 10)
```
**Solution**: Vector memory exhausted, So I can only write the code, but cant run the result.
...
```{r}
mimic_icu_cohort %>%
  ggplot(aes(x = factor(ethnicity), fill = thirty_day_mort)) +
    geom_bar(position = "fill") +
    scale_fill_manual(values = c("blue", "yellow")) +
    labs(x = "Ethnicity", y = "Proportion", fill = "Thirty Day Mortality") +
    theme(legend.position = "bottom") +
    facet_wrap(~thirty_day_mort, scales = "free_y")
```
